 ...
 *
 * Minimal TLS 1.2 client that completes a handshake (approved SNI, ALPN
 * http/1.1 — the worker-facing broker enforces both post-gate) and then
 * attempts client-initiated renegotiation, verifying the worker-facing
 * server context refuses it (OP_NO_RENEGOTIATION, measured effective in
 * the M4B-2 ECH spike, E4).  Ported from spikes/m4b2-ech-gate/
 * reneg_client.c with the ALPN offer added.  Hand-declared prototypes:
...
/* ALPN wire format: one length-prefixed protocol name ("http/1.1"). */
...
int main(int argc, char **argv)
{
    ...
    SSL_CTX_set_alpn_protos(ctx, ALPN_PROTOS, (unsigned int)(sizeof(ALPN_PROTOS) - 1));
    ssl = SSL_new(ctx);
    SSL_set_fd(ssl, fd);
