"""Synthetic TLS ClientHello generator for the M4B-2 ECH gate spike.

SPIKE CODE — not production. Standard-library only. Builds raw TLS bytes so
conformance tests can present arbitrary (including hostile/malformed)
ClientHello messages to a candidate ECH gate and to a real OpenSSL server.
